\documentclass[11pt]{amsart}

\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{amsthm}
%\usepackage{psfig}

\begin{document}
\baselineskip=15pt
\textheight=8.4in
\parindent=0pt
\def\sk {\hskip .5cm}
\def\skv {\vskip .12cm}
\def\cos {\mbox{cos}}
\def\sin {\mbox{sin}}
\def\tan {\mbox{tan}}
\def\intl{\int\limits}
\def\lm{\lim\limits}
\newcommand{\frc}{\displaystyle\frac}
\def\xbf{{\mathbf x}}
\def\fbf{{\mathbf f}}
\def\gbf{{\mathbf g}}

\def\Ker{{\rm Ker\,}}
\def\phi{\varphi}

\def\dbA{{\mathbb A}}
\def\dbB{{\mathbb B}}
\def\dbC{{\mathbb C}}
\def\dbD{{\mathbb D}}
\def\dbE{{\mathbb E}}
\def\dbF{{\mathbb F}}
\def\dbG{{\mathbb G}}
\def\dbH{{\mathbb H}}
\def\dbI{{\mathbb I}}
\def\dbJ{{\mathbb J}}
\def\dbK{{\mathbb K}}
\def\dbL{{\mathbb L}}
\def\dbM{{\mathbb M}}
\def\dbN{{\mathbb N}}
\def\dbO{{\mathbb O}}
\def\dbP{{\mathbb P}}
\def\dbQ{{\mathbb Q}}
\def\dbR{{\mathbb R}}
\def\dbS{{\mathbb S}}
\def\dbT{{\mathbb T}}
\def\dbU{{\mathbb U}}
\def\dbV{{\mathbb V}}
\def\dbW{{\mathbb W}}
\def\dbX{{\mathbb X}}
\def\dbY{{\mathbb Y}}
\def\dbZ{{\mathbb Z}}


\def\char{{\rm char}}
\def\wt{{\rm wt}}
\def\Aut{{\rm Aut}}
\def\Hom{{\rm Hom}}
\def\End{{\rm End}}
\def\Mat{{\rm Mat}}
\def\Irr{\mathrm {Irr}}
\def\Im{{\rm Im}}
\def\GL{{\rm GL}}
\def\SL{{\rm SL}}
\def\SNF{{\rm SNF}}
\def\tr {{\rm tr}}
\def\rk{{\rm rk}}
\def\lam{{\lambda}}

\def\la{{\langle}}
\def\ra{{\rangle}}

\bf
\begin{center}
{Homework \# 9, due on Canvas by 11:59pm on Fri, Apr 10th.}\rm
\end{center}
\vskip .2cm
\rm

{\bf Plan for the next 2 classes:} Separable extensions and the Primitive Element Theorem. Start talking about Galois groups.
References: 13.5 and 14.1 in Dummit and Foote, Lectures 17-19 from Spring 2010 and Lectures 12-15 from Spring 2021.
\vskip .1cm

%{\bf Note on hints:} Some hints are given at the end of the assignment, each on a separate page.
%Problems (or parts of problems) for which a hint is available at the end are marked with *.

\skv
{\bf 1.} Let $\sigma$ be an action of a group $G$ on a finite set $X$, and let $(\dbC X)_{\sigma}$ be the corresponding
permutation representation of $G$ over $\dbC$ (here $\dbC X$ is the underlying space and the subscript $\sigma$ is just
used to explicitly refer to $\sigma$ in the notation). The representation $\dbC X$ decomposes as a direct sum of two subrepresentations:
$(\dbC X)_{\sigma} = W_{\sigma}\oplus W_{\sigma}^{\perp}$ where $W_{\sigma}=\dbC(\sum\limits_{x\in X}x)$ and $
W_{\sigma}^{\perp}$ is the orthogonal complement
with respect to the Hermitian dot product on $\dbC X$ (under the natural identification between $\dbC X$ and $\dbC^{|X|}$),
that is, $$W_{\sigma}^{\perp}=\{\sum \lam_x x\in \dbC X: \sum \lam_x =0\}$$ (note that if $G=S_n$ and $\sigma$ is the definining action
of $G$ on $\{1,\ldots, n\}$, then $W_{\sigma}^{\perp}$ is the standard representation of $S_n$), The main goal of this problem is to
find a simple criterion for the representation $W_{\sigma}^{\perp}$ to be irreducible.
\skv
Going back to the original setup, let $\sigma$ be an action of a group $G$ on a set $X$ with $|X|>1$. 
\begin{itemize}
\item Denote by $\sigma^2$ the {\it diagonal} action of $G$ on $X\times X$ given by $g.(x,y)=(gx,gy)$.  
\item Note that $\sigma^2$ stabilizes the diagonal $D(X)=\{(x,x)\in X\}\subset X\times X$ and hence also 
$(X\times X)\setminus D(X)$. Denote by $\sigma(2)$ the restriction of $\sigma^2$ to $(X\times X)\setminus D(X)$.
\end{itemize}
The {\it rank} of $\sigma$ (denoted $\rk(\sigma)$) is defined to be the number of orbits of $\sigma(2)$.
Note that $\rk(\sigma)=1$ $\iff$ $\sigma$ is {\it 2-transitive}, that is, if $G$ acts transitively 
on the ordered pairs of distinct elements of $X$.
\skv
Now the actual problem begins. Recall that if $V$ is a representation of $G$, by $V^G$ we denote the subspace of global fixed points.
\begin{itemize}
\item[(a)] Let  $\tau$ be an action of a group $G$ on a set $X$, and let $F$ be a field.
Prove that $\dim((FX)_{\tau}^G)$ is equal to the number of orbits of $\tau$. {\bf Hint:} We proved this in class in the case of 
the conjugation action of $G$ on itself. The proof in the general case is analogous.
\item[(b)] Now assume that $\sigma$ is an action of a finite group $G$ on a finite set $X$, and let
$\chi_{\sigma}$ be the character of the $(\dbC X)_{\omega}$. Prove that 
$$\la \chi_{\sigma},\chi_{\sigma}\ra=\frac{1}{|G|}\sum\limits_{g\in G}\chi_{\sigma^2}(g).$$
\item[(c)] Now applying (a) to $\tau=\sigma^2$ (and $F=\dbC$) and a suitable result from Lecture~15, deduce
that $\la \chi_{\sigma},\chi_{\sigma}\ra$ is equal to the number of orbits of $\sigma^2$. 
\item[(d)] Finally use (c) to prove that $W^{\perp}_{\sigma}$ (as defined above)
is irreducible if and only if $|X|=2$ or $\rk(\sigma)=1$. {\bf Hint:} Consider separately the cases where
$\sigma$ is transitive and $\sigma$ is not transitive.
\end{itemize}



{\bf 2.} Let $p$ be a prime, and let 
$G$ be the set of all functions from $\dbF_p$ to $\dbF_p$ which have the form
$x\mapsto ax+b$ for some $a\in\dbF_p^{\times}$ and $b\in\dbF_p$.
\begin{itemize}
\item[(a)] Prove that $G$ is a group (with respect to composition) isomorphic to the group of matrices $\left\{\begin{pmatrix} a&b\\ 0&1\end{pmatrix}:
a\in\dbF_p^{\times}, b\in\dbF_p\right\}$. Note that for $p=5$ this is the group
from Problem~3 on Midterm \#2.
\item[(b)] The group $G$ has a natural action on $\dbF_p$ (given by
$g.x=g(x)$ for all $g\in G$ and $x\in\dbF_p$). Prove that this action is
$2$-transitive. By Problem~1, this gives a more conceptual proof of the irreducibility of the $4$-dimensional representation
you had to construct in Problem~3 on Midterm \#2. 
\end{itemize}
\skv
\skv
{\bf 3.} \rm 

\begin{itemize}

\item[(a)] Let $K=\dbQ(\sqrt{2},\sqrt{3})$. Prove that $[K:\dbQ]=4$.

\item[(b)] Let $L=\dbQ(\sqrt{2}+\sqrt{3})$. Prove that $L=K$ and hence $[L:\dbQ]=4$ by (a).
\end{itemize}

{\bf 4.} \rm Let $S=\{n_1,\ldots, n_k\}$ be a finite set of positive integers $\geq 2$
and let $K=\dbQ(\sqrt{n_1},\ldots, \sqrt{n_k})$.
\begin{itemize}
\item[(a)] Prove that $[K:\dbQ]=2^m$ for some $m\leq k$ and the set $P(S)=\{1\}\cup \{\sqrt{n} : n \mbox{ is a product of distinct elements of }S\}$ spans $K$ over $\dbQ$.

\item[(b)] For each $0\leq j\leq k$ let $\dbQ_j=\dbQ(\sqrt{n_1},\ldots, \sqrt{n_j})$ (we set $\dbQ_0=\dbQ$).
Prove that $[K:\dbQ]<2^k$ if and only if $n_1$ is a perfect square or
there exists $2\leq i\leq k$ s.t. $\sqrt{n_i}=a+b\sqrt{n_{i-1}}$ for some $a,b\in\dbQ_{i-2}$.

\item[(c)] Assume that the elements of $S$ are pairwise relatively prime
and none of them is a complete square.
Prove that $[K:\dbQ]=2^k$. {\bf Hint:} Use (b) and induction on $k=|S|$.
\end{itemize}

{\bf 5.} \rm Let $F$ be a field, and let $\alpha$ be an algebraic element of odd degree
over $F$ (recall that the degree of $\alpha$ over $F$ is equal to $[F(\alpha):F]$). Prove that
$F(\alpha^2)=F(\alpha)$.
\skv
\skv
{\bf 6.} Let $K/F$ be a finite field extension, $n=[K:F]$, and fix some basis
$\Omega=\{\alpha_1,\ldots, \alpha_n\}$
for $K$ over $F$. For any $\alpha\in K$
define $T_{\alpha}:K\to K$ by $T_{\alpha}(\beta)=\alpha\beta$. Note that $T_{\alpha}\in  \End_F(K)$.
Let $A_{\alpha}=[T_{\alpha}]_{\Omega}\in Mat_n(F)$ be the matrix of $T_{\alpha}$ with respect to $\Omega$.
\begin{itemize}


\item[(a)] (practice) Prove that the map $K\to \Mat_n(F)$ given by $\alpha\mapsto A_{\alpha}$ is an injective ring homomorphism.
\item[(b)] Prove that the minimal polynomial of $\alpha$ over $F$ and the minimal polynomial of $A_{\alpha}$
coincide.

\item[(c)] Find the minimal polynomial of the matrix
$$A=\begin{pmatrix}  0&3& 5&0\\
1&0& 0&5\\
1&0& 0&3\\
0&1& 1&0\\
\end{pmatrix}$$
without doing extensive computations.
{\bf Hint:} Find an extension $K/\dbQ$ of degree $4$, a basis for $K$ over $\dbQ$
and an element $\alpha\in K$ such that $A_{\alpha}=A$ in the notations of part (a).
\end{itemize}

\skv
{\bf 7. (practice)} \rm Let $F$ be a field and $\Omega$ a subset of $F[x]$.
Use the existence and uniqueness of algebraic closures to prove that
there exists a unique splitting field for $\Omega$ over $F$ up to
$F$-isomorphism. {\bf Hint:} Use the fact that any splitting field 
$K$ for $\Omega$ lies in some algebraic closure of $F$ (this was proved in Lecture~21 on April 2).
\skv

{\bf 8.} Let $K/F$ be a field extension, and let $K_1$ and $K_2$
be subfields of $K$ containing $F$ such that the extensions
$K_1/F$ and $K_2/F$ are normal. Prove that the extensions
$K_1 K_2/F$ and $K_1\cap K_2/F$ are also normal.
Here $K_1 K_2$ is the {\it composite field of $K_1$ and $K_2$} (see  pp. 528-529 of DF for the definition).
\skv

\newpage
{\bf Hint for 3:} Use our discussion in Lecture~15 to find a general condition on a finite group $G$ which guarantees that all of its complex characters are real-valued and then show that this condition holds for $S_n$.
\end{document}
