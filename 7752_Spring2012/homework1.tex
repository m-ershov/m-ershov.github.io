...
Reading: 10.4 in DF and Lectures 3, 4, 5 from the webpage. Note that
the first part of Lecture 5 was discussed in the last class of
Algebra-I.
\vskip .1cm

\bf\centerline{Problems, to be submitted by Thu, January 26th. }\rm
\vskip .1cm

{\bf Convention:} All rings below are assumed to have $1$, and all modules are left modules.
\skv
\vskip .1cm
{\bf Problem 1: } Let $R$ be a commutative Noetherian ring and $\phi:R\to R$
a surjective ring homomorphism. Prove that $\phi$ must be an isomorphism.
{\bf Hint:} Consider the ideals $\Ker(\phi^n),\, n\in\dbN$.
\vskip .2cm
\bf{Problem 2 (practice): }\rm Let $M$ be an $R$-module for some ring $R$ (not necessarily commutative).
\begin{itemize}
...
\end{itemize}
\bf{Problem 3: }\rm Let $R$ be a ring and let $M$ be an $R$-module.
\begin{itemize}
...
\item[(b)] Assume that $R$ is commutative, and let $M$ be an $R$-module. Prove that $Hom_R(R,M)\cong M$ as $R$-modules. {\bf Hint:}
An element of $Hom_R(R,M)$ is uniquely determined by where it maps $1$.
\end{itemize}
\bf{Problem 4: }\rm In Lecture 27 we obtained a simple characterization of $R$-modules
for $R=\dbZ$ and $R=F[x]$, with $F$ a field. 
\begin{itemize}
...
\end{itemize}
\skv
\bf{Problem 5 (practice): }\rm Let $G$ be a group and $\dbZ[G]$ its integral group ring
...
\bf{Problem 6: }\rm An $R$-module $M$ is called {\it simple (or irreducible)} if
...
\bf{Problem 7: }\rm  An $R$-module $M$ is called {\it cyclic} if $M$ is generated
...
{\bf Problem 8.} Let $R$ be a commutative domain, and let $I$ be a non-principal ideal of $R$. 
...
\vskip .12cm
{\bf Problem 9.} Prove Schur's lemma [DF, problem 11, p.356].
\end{document}
